...
class Command(BaseCommand):
    help = 'Lista imagens velhas de fansubs'
    ...
    def handle(self, *args, **options):
        prefix = Fansub._meta.get_field('img').upload_to
        if prefix.endswith('/'):
            prefix = prefix[:-1]
        ...
        images_saved = Fansub.objects.filter(img__startswith=prefix).values_list('img', flat=True)
